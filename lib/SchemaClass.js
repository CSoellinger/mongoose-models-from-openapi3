...
class SchemaClass {
    ...
    static getMongooseSchema(properties, requiredSchemaFields) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let propertyName in properties) {
                if (properties[propertyName]) {
                    ...
                    switch (property.type) {
                        case 'number':
                        case 'integer':
                            mongooseFieldSchema = Object.assign(mongooseFieldSchema, SchemaClass.getNumberSchema(property));
                            break;
                        case 'string':
                            mongooseFieldSchema = Object.assign(mongooseFieldSchema, SchemaClass.getStringSchema(property));
                            break;
                        case 'boolean':
                        ...
                        case 'array':
                            if (property.items.type === 'object') {
                                if (Object.keys(property.items.properties).indexOf('_id') !== -1) {
                                    mongooseFieldSchema.type = [new mongoose.Schema(property.items.properties)];
                                }
                            }
                            ;
                            break;
            ...
            console.log('mongooseSchema', mongooseSchema, properties);
